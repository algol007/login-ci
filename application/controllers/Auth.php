...
class Auth extends CI_Controller
{
    ...
    private function _sendEmail($token, $type)
    {
        ...
        $config = [
            ...
            'smtp_user' => 'ard.advertising2k18@gmail.com',
            'smtp_pass' => 'ARcomp2k18',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charser' => 'utf-8',
            'newline' => "\r\n"
        ...
        $this->email->from('ard.advertising2k18@gmail.com', 'AR Digital Ads');
        $this->email->to($email);

        if ($type == 'verification') {
            $this->email->subject('Email Verification');
            $this->email->message('Click this link to make your account active : <a href="' . base_url() . 'auth/activation?email=' . $email . '&token=' . urlencode($token) . '">Activate your account!</a>');
        } else if ($type == 'forgot') {
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->db->delete('tb_user_token', ['token' => $token]);
    ...
    public function forgotPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $email = $this->input->post('email');
            $user = $this->db->get_where('tb_user', ['email' => $email])->row_array();
            $active = $this->db->get_where('tb_user', ['email' => $email, 'is_active' => 1])->row_array();
            ...
            if ($user) {
                if ($active) {
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
                    Your email is not active! Please activated your email first.
                    </div>');
                    redirect('auth/forgotpassword');
                }
            } else {
